...
-------------------------------------------------------------------------------
   ...
   Version 0.1 -- September 6, 2019 -- Marc Costa Sitja, ESAC/ESA

      Updated IDs for all the frames.
...
-------------------------------------------------------------------------------
   ...
   The following frame is defined in this kernel file:
      ...
      ------------------------  ---------------------------------------------
      HERA_DIDYMAIN_NPO         Hera Didymain Nadir power-optimized pointing
   ...
   ID:
      ...
      ------------------------  ---------------------  -------  ---------
      HERA_DIDYMAIN_NPO         HERA                   DYNAMIC   -999010
...
-------------------------------------------------------------------------------
   ...
   used in two different ways:
      ...
      [2] In combination with the Hera Frames kernel, to define
          ...
          "Nadir power optimized with respect to Didymain", then the furnish
          ...
          kernels, in the following order:
              ...
              $DATA/fk/hera_sc_didymain_npo_v00.tf
          ...
          By loading the ``hera_sc_didymain_npo_vNN.tf'' frames kernel last,
          ...
          the HERA_DIDYMAIN_NPO frame defined in the ``Hera Science
...
Hera Didymain Nadir power-optimized pointing frame (HERA_DIDYMAIN_NPO)
------------------------------------------------------------------------

   Definition:
   -----------
   The Hera Didymain Nadir power-optimized pointing frame is defined as
   follows:

      -  -Y axis is the primary vector and points from Hera to the
         center of Didymain (Nadir direction);
   ...
   required to compute both the Hera-Didymain position and the Hera-Sun
   ...
   based on the Hera-Didymain position and Hera-Sun position vectors, the usage
   of different ephemerides to compute these vectors may lead to different
   frame orientation at given time.

   \begindata

      FRAME_HERA_DIDYMAIN_NPO        = -999010
      FRAME_-999010_NAME            = 'HERA_DIDYMAIN_NPO'
      FRAME_-999010_CLASS           =  5
      FRAME_-999010_CLASS_ID        = -999010
      FRAME_-999010_CENTER          = -999
      FRAME_-999010_RELATIVE        = 'J2000'
      FRAME_-999010_DEF_STYLE       = 'PARAMETERIZED'
      FRAME_-999010_FAMILY          = 'TWO-VECTOR'
      FRAME_-999010_PRI_AXIS        = 'Z'
      FRAME_-999010_PRI_VECTOR_DEF  = 'OBSERVER_TARGET_POSITION'
      FRAME_-999010_PRI_OBSERVER    = 'HERA'
      FRAME_-999010_PRI_TARGET      = 'DIDYMAIN'
      FRAME_-999010_PRI_ABCORR      = 'NONE'
      FRAME_-999010_SEC_AXIS        = 'X'
      FRAME_-999010_SEC_VECTOR_DEF  = 'OBSERVER_TARGET_POSITION'
      FRAME_-999010_SEC_OBSERVER    = 'HERA'
      FRAME_-999010_SEC_TARGET      = 'SUN'
      FRAME_-999010_SEC_ABCORR      = 'NONE'
      FRAME_-999010_SEC_FRAME       = 'J2000'
